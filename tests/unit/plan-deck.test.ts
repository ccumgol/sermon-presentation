...
import {
  buildPlanDeck, buildPlanRows, describeItem, insertIndexFor, isExpandable, itemsInGroup, splitOrderText, moveItem, removeItem, type ItemResolver } from '../../lib/plan-deck.ts';
...
describe('splitOrderText — 순서 이름과 담당자', () => {
   ...
   *
   * 전에는 한 줄로 합쳤다. 그래서 이름과 직분을 나눠 쳐도 화면에는 붙어 나갔고,
   * 길면 자리가 넘쳤다 — '김애리 전도사 / MD연합여선교회 증경회장' 처럼.
   * 이제 접을지 줄일지를 **사람이 정한다**: 두 줄로 치면 두 줄, 한 줄로 치면
   * 넘칠 때 글자가 줄어든다(출력 화면의 `fitPresenterWidth`).
   */
  it('담당자가 여러 줄이면 줄바꿈을 그대로 싣는다', () => {
  ...
  /** 사용자가 든 예 1 — 두 줄로 치면 두 줄로 나간다 */
  it('담당자와 직분을 나눠 치면 두 줄이다', () => {
    expect(splitOrderText('성경봉독\n김애리 전도사\nMD연합여선교회 증경회장')).toEqual({
      title: '성경봉독',
      presenter: '김애리 전도사\nMD연합여선교회 증경회장',
    });
  });

  /** 사용자가 든 예 2 — 한 줄로 치면 한 줄이다 (넘치면 화면이 줄인다) */
  it('한 줄로 치면 한 줄 그대로다', () => {
    expect(splitOrderText('성경봉독\n김애리 전도사 / MD연합여선교회 증경회장')).toEqual({
      title: '성경봉독',
      presenter: '김애리 전도사 / MD연합여선교회 증경회장',
    });
  });

  /** 사이의 빈 줄은 버린다 — 빈 줄이 그대로 실리면 밑줄만 한 칸 내려간다 */
  it('사이에 빈 줄이 있어도 줄이 늘지 않는다', () => {
