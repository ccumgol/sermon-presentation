...
describe('찬양', () => {
  /**
   * **곡집·번호를 빼고 제목만** (2026-09-12 사용자 요청).
   *
   * '많은물소리 265장 이 땅의 황무함을 보소서' 처럼 나가던 것이 요청의 계기다.
   * 회중에게 곡집 이름과 번호는 뜻이 없고, 앞에 붙은 글자가 길수록 제목이 작아진다.
   * 교독문·주기도문이 이미 같은 규칙이다 — 찬양만 예외였다.
   */
  /**
   * **찬송가만 번호를 적는다** (2026-09-12 사용자 결정).
   *
   * 가르는 기준은 **회중이 손에 든 책이 있는가**다. 찬송가는 번호로 펴야 하니
   * 번호가 일이고, 경배와찬양 계열은 책이 없어 번호가 글자만 차지한다.
   */
  it('찬송가는 번호를 함께 띄운다', () => {
    for (const songbookId of ['hymn_new', 'hymn_old']) {
      expect(
        itemTitle({
          id: 'a',
          type: 'song',
  ...
  /** 사용자가 든 예 — 많은물소리 265장은 회중에게 책이 없다 */
  it('찬송가가 아니면 제목만', () => {
    expect(
      itemTitle({
        id: 'a',
        type: 'song',
        songId: 1,
        songTitle: '이 땅의 황무함을 보소서',
        songLabel: '많은물소리 265장',
        songbookId: 'many_waters',
        langs: ['ko'],
      }),
    ).toBe('이 땅의 황무함을 보소서');
  });
  ...
  /**
   * **옛 순서표에는 `songbookId` 가 없다.** 모를 때는 빼는 쪽이 맞다 —
   * 찬송가가 아닌 번호가 화면에 나가는 것이 이 변경의 계기였다.
   */
  it('곡집을 모르면 제목만 — 옛 순서표', () => {
    expect(
      itemTitle({
        id: 'a',
        type: 'song',
