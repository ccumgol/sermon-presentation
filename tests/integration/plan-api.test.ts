...
describe('데이터 이전', () => {
  ...
  /**
   * ⏱ **이 검사만 시간 제한을 늘려 둔다** (기본 5초 → 30초).
   *
   * 2026-09-09 에 CI 를 세우자마자 여기서 걸렸다. 실측:
   *
   * | 어디 | 이 파일 42개 |
   * |---|---:|
   * | GitHub 러너 · Node 26 | **750 ms** |
   * | GitHub 러너 · Node 24.20.0 | **14,420 ms** ← 5초 초과 |
   * | 맥(로컬) · Node 24.16.0 | 604 ms |
   * | 맥(로컬) · Node 26.7.0 | 463 ms |
   *
   * **리눅스 러너의 Node 24 에서만** 느리다 — 맥에서는 두 판이 같다. 그 러너에서
   * DB 쓰기가 많은 검사(`songbooks-api` · `ws-hub`)도 4배쯤 느려지므로
   * `node:sqlite` 쓰기 쪽으로 보인다. 400곡을 진짜로 넣는 이 검사가 가장 크게 받는다.
   *
   * **설치판에는 영향이 없다.** Electron 이 싣는 Node 24 는 맥·윈도우에서 도는데,
   * 맥 실측이 604ms 다. 그래서 고칠 것이 아니라 **기다려 주면 되는 것**으로 본다.
   *
   * 전역 `testTimeout` 을 올리지 않는 이유: 진짜로 멈춘 검사까지 30초씩 기다리게 된다.
   */
