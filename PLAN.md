...
헬라어가 특수문자처럼 자간이 벌어져 나온다는 보고. **데이터는 정상**이었다 —
정규 다음절 그리스어(`Ἐ` U+1F18, `ῇ` U+1FC7)에 공백도 하나씩이었다.

원인은 폰트 체인이 총칭 `serif` 로만 끝난 것. 전용 폰트가 하나도 설치돼 있지 않아
(SBL Greek·Cardo·Gentium Plus·SBL Hebrew 전부 미설치) 총칭으로 떨어졌고, 그 폰트가
U+1F00 대역을 덮지 못해 **글자마다 다른 폰트로 대체**됐다.

측정으로 확인한 판별법 — 다음절 폭 ÷ 단음절 폭 비율:

| 폰트 | 비율 | 판정 |
|------|-----:|------|
| `serif` (실제 사용됨) | — | 819.6px |
| Apple SD Gothic Neo | 1.10 | 글자별 대체 |
| Times New Roman | 1.02 | 자체 글리프 ✓ |
| Baskerville · Didot | 1.02 | ✓ |

총칭 앞에 Times New Roman 을 넣어 819.6px → 526.6px(36% 감소), 비율 1.02 로 정상화.
히브리어는 부호가 자음 위에 얹혀 폭이 늘지 않아(비율 1.00) 원래 티가 안 났지만
같은 이유로 실제 폰트를 앞에 뒀다.

**프리셋만 고쳐서는 안 됐다.** 프리셋은 코드에서 생성되지만 사용자가 저장한 사본은
JSON 으로 굳어 있어 옛 체인이 그대로였고, 실제로 활성 템플릿이 그런 사본이었다.
저장소 초기화 때 **옛 문자열과 정확히 같을 때만** 새 체인으로 올린다 —
직접 고른 폰트는 문자열이 다르므로 건드리지 않는다.

주석에 "출력 페이지가 실제 사용된 폰트를 측정해 경고한다"고 적혀 있었으나
**그 기능은 존재하지 않았다.** 있었다면 이 문제를 바로 알 수 있었을 것이다.
주석을 사실대로 고쳤다.
