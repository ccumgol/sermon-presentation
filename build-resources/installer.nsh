...
;
; ⚠️ **`!ifndef BUILD_UNINSTALLER` 로 감싸야 한다** (2026-09-10 CI 가 잡았다).
; electron-builder 는 설치 관리자와 **제거 관리자를 따로 컴파일**하는데,
; `customPageAfterChangeDir` 는 설치 쪽에만 끼워진다. 감싸지 않으면 제거 관리자
; 컴파일에서 이 함수들이 아무 데서도 안 불려
;   `warning 6010: install function "PrereqPageShow" not referenced`
; 가 나고, electron-builder 는 **경고를 오류로 다룬다** → 빌드가 깨진다.
