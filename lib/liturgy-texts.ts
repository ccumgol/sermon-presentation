 ...
 * ## 줄바꿈은 데이터에 박아 둔다
 *
 * 찬양 가사는 원본이 한 덩어리라 자동으로 잘라야 했지만(`lib/meter-align.ts`),
 * 이 본문들은 **회중이 소리 내어 함께 읽는다.** 끊는 자리가 곧 호흡이므로
 * 자동 분할에 맡기지 않고 의미 단위로 미리 끊어 둔다.
 *
 ...
 * ## 교회마다 다를 수 있다
 *
 * 여기 담은 것은 **널리 쓰이는 판본**이지 유일한 정답이 아니다. 특히 전통 판본은
 * 교회마다 '나라이/나라가' 처럼 갈리는 자리가 있다. 그래서 항목에 직접 고친 본문을
 * 담을 수 있게 해 두었다 (`CueItem` 의 `overrideLines`). 고친 본문이 있으면 그것이 이긴다.
 */
 ...
 * 항목이 실제로 띄울 줄들.
 *
 * 직접 고친 본문(`overrideLines`)이 있으면 그것이 이긴다 — 교회 판본이 다를 때
 * 사용자가 손본 것을 자동 데이터가 덮어쓰면 안 된다 (찬양의 '승인'과 같은 원칙).
 */
