...
/**
 * 출력 페이지에 넘겨도 되는 스타일 키인가 (점검 S-3 · 보안 감사 L-1).
 *
 * ## 무엇을 겪었나 (2026-09-07 실측)
 *
 * `style:set` 이 아무 키나 통과시켰다. 격리 서버에 `{opacity:0, display:none}` 을
 * 보내니 걸러지지 않고 출력 페이지까지 갔고, 거기서
 * `root.style.setProperty('display', 'none')` 이 되어 **송출 화면이 통째로 사라진다.**
 *
 * 더 나쁜 것은 **되돌릴 수 없다**는 점이다 — 템플릿을 다시 보내도 `--` 로 시작하는
 * 변수만 덮으므로 `display` 는 그대로 남는다. OBS 소스를 새로고침해야 한다.
 ...
 * | 키 | 왜 |
 * |---|---|
 * | `--` 로 시작 | 사용자 정의 속성. 우리 CSS 가 읽는 값이고, 모르는 이름은 무시된다 |
 * | `backdrop` | 그림 배경은 요소를 만들어야 해서 CSS 변수로 표현할 수 없다 |
 * | `anchor` | 출력 페이지가 `--justify`/`--align` 으로 풀어 쓴다 (옛 보내는 쪽 호환) |
 * | 그 밖 | **막는다** — 진짜 CSS 속성이면 화면 자체를 바꿀 수 있다 |
 *
 * 프리셋 8종을 실측해 정당한 키 81개 중 `--` 로 시작하지 않는 것은 `backdrop`
 * 하나임을 확인했다. 그래서 이 규칙은 지금 동작하는 것을 하나도 막지 않는다.
 *
 * ⚠️ **같은 규칙이 `public/output/output.js` 에도 있다.** 출력 페이지는 의존성 0
 * 이라(OBS 내장 브라우저 대비) 이 파일을 가져다 쓸 수 없다. 고칠 때 양쪽을 함께
 * 고친다 — 서버만 고치면 다른 경로로 보낸 값이 그대로 화면에 들어간다.
 */
export function isAllowedStyleKey(key: string): boolean {
  return key.startsWith('--') || key === 'backdrop' || key === 'anchor';
}
