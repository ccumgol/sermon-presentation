...
export interface SheetRef {
  ...
  /**
   * 몇 번째 단인가 (1부터). **화면에 보여 주기 위한 것**이고 그리는 데는 쓰지 않는다.
   *
   * 조작자가 슬라이드 목록에서 '이 줄은 3단' 을 볼 수 있어야 배분이 어긋난 것을
   * 알아챈다. 퍼센트만 보고는 몇 번째인지 셀 수 없다.
   */
  system: number;
  /** 이 악보에 단이 모두 몇 개인가 — `3/8단` 처럼 보여 주려면 필요하다 */
  systemCount: number;
  /**
   * 배분이 흔들릴 수 있는 곡인가.
   *
...
export type CueItem =
  ...
  | {
      ...
      lines?: string;
      /**
       * 프로젝터에 **악보**를 낸다 (`true`). 없거나 `false` 면 가사다.
       *
       * **기본이 가사인 이유**(2026-09-04 사용자 결정): 단 경계 자동 검출이 아직
       * 불완전하다. 틀린 자리가 벽에 걸리는 것보다 가사가 낫다 — 가사는 늘 맞다.
       *
       * 악보가 있는 곡집은 지금 **찬미예수 2000 뿐**이다. 켜 두어도 악보가 없는
       * 곡은 그냥 가사가 나간다.
       *
       * OBS 출력과 강사 모니터는 이 값과 무관하게 **언제나 가사**다.
       */
      sheet?: boolean;
      /**
...
export type SlidePayload =
  ...
  | {
      kind: 'song';
      /**
       * 어느 곡인가 (2026-09-12 사용자 요청).
       *
       * 예배 중에 오타를 발견하면 **찬양 탭에서 곧바로 고칠 수 있어야 한다.**
       * 전에는 덱이 제목만 싣고 있어서, 조작자가 같은 곡을 제목으로 **다시 검색해**
       * 들어가야 했다 — 제목이 비슷한 곡이 여럿이면 그마저 헷갈린다.
       *
       * 선택 항목이다 — 저장된 옛 덱(`live_state`)에는 이 칸이 없다.
       */
      songId?: number;
      title: string;
      sectionLabel: string;
...
export interface DeckGroup {
  ...
  startIndex: number;
  /**
   * 이 경계를 만든 예배 순서 항목의 id.
   *
   * **자리 번호로 짝지으면 안 되기 때문에 있다.** 못 푼 항목은 `groups` 에서 빠지므로
   * `groups[i]` 와 `항목[i]` 가 어긋난다 — 앞에 실패가 하나만 있어도 그 뒤가 전부 밀린다.
   *
   * 지금 두 곳이 이것을 쓴다: 다시 올릴 때 **보던 자리 찾기**, 그리고 경계에 실을
   * **템플릿 고르기**. 옛 덱에는 없을 수 있으므로 없을 때도 동작해야 한다.
   */
  itemId?: string;
  /**
