 ...
 * 배포판이 나간다.
 *
 * **아이콘을 고칠 때만 필요하다.** 만들어진 파일은 저장소에 커밋되므로, 앱을
 * 빌드하거나 쓰는 데는 이 도구들이 필요 없다 (윈도우 CI 도 커밋된 것을 쓴다).
 *
 * 전에는 프로젝터 PWA 아이콘을 `make-projector-icons.ts` 가 `node:zlib` 만으로
 * 따로 그렸다 — 도구를 안 깔아도 되게 한 것이었다. 앱 아이콘이 생기면서 **같은
 * 파일을 둘이 만들게 되어** 어느 쪽이 원본인지 알 수 없어졌고, 하나로 합쳤다
 * (2026-09-05).
 */
